...
import { createAppTnc } from "@/store/apps/apptnc";
...
export default function AddNewAppTncPageView() {
  ...
  async function handleFormSubmit(values) {
    try {
      ...
      const response = await dispatch(createAppTnc(values)).unwrap();
      console.log("res app TNC", response);
      if (response?.status === 201) {
        toast.success("app tnc created successfully!");
        navigate("/apptnc-list-2");
      } else {
        console.log("response", response);
        toast.error(response?.message || "Failed to create app Terms and Conditions");
      }
    } catch (error) {
    ...
    <div className="pt-2 pb-4">
             <FlexBox mb={2} alignItems="center">
              ...
              <Paragraph fontSize={18} fontWeight="bold">
                Add New App Terms and Conditions
              </Paragraph>
            </FlexBox>
      <Card sx={{ p: 3 }}>
