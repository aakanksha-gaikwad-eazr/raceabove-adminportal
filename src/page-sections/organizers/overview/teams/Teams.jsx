...
export default function Teams({allDataOfSingleUser}) {
  ...
  const iconList = [KeyframeBezierIn, PaletteOutlined, InvertColors];
  const colorList = [
  ...
  const TEAM_LIST  = allDataOfSingleUser?.challengeParticipations || []
  ...
  return <Card className="p-3">
      ...
      <Scrollbar autoHide={false}>
        ...
        minWidth: 600
          ...
          <TableBody>
            {TEAM_LIST.map((participation,index) => {
                ...
                <TableCell>
                  <Stack mb={1} alignItems="center" direction="row" spacing={2}>
                    ...
                    <div>
                      ...
                      <Paragraph color="text.secondary">{challenge?.description}</Paragraph>
                    </div>
                  </Stack>
                </TableCell>

                <TableCell>Starting On {challenge?.startDate}</TableCell>

                <TableCell>
                  <Stack direction="row" justifyContent="flex-end" spacing={2}>
                    <StyledAvatarGroup max={3}>
                      <Avatar src={challenge?.badge} />
