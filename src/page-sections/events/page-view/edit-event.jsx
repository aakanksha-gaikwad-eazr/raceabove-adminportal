...
export default function EditEventPageView({ open, handleClose }) {
        ...
        return (
          <Stack spacing={2}>
            ...
            <Dropzone
              ...
              preview={formik.values.bannerFile ? null : eventData?.banner}
            />
            ...
            <TextField
              ...
              value={values.coinsDiscountPercentage || 0}
              onChange={handleChange}
            />
          </Stack>
        );
        ...
        return (
          <Stack spacing={2}>
            ...
            <TextField
              name="participation"
              label="Participation"
              value={values.participation || "N/A"}
              onChange={handleChange}
            />
            ...
            <TextField
              name="enduranceLevel"
              label="Endurance"
              value={values.enduranceLevel || "N/A"}
              onChange={handleChange}
            />
        ...
        return (
          <Stack spacing={2}>
            {values.slots.map((slot, index) => (
              <Stack direction="row" spacing={2} key={slot?.id || `slot-${index}`}>
                <TextField
                  label="Start Time"
          ...
          <Stack spacing={2}>
            <Select
              ...
              renderValue={(selected) =>
                selected
                  .map((couponarr) => { 
                    console.log("dg",couponarr.code)
                   return (coupons.find((c) => c.id === couponarr.id)?.code || "Unknown") })
                  .join(", ") 
              }
            >
              {coupons.map((coupon) => (
                <MenuItem key={coupon.id} value={coupon.id}>
                  ...
                  <ListItemText
                    primary={`${coupon?.code} (${coupon?.description})` || 'N/A'}
                  />
                </MenuItem>
              ))}
            </Select>
