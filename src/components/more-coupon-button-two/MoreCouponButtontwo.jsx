...
export default function MoreCouponButtontwo({
  couponId,
  size = "large",
  Icon = MoreVert,
  options = optionList,
  ...
  const handleOtpions = (option, couponId) => {
    if (option === "Edit") {
      navigate(`/edit-coupons/${couponId}`); // <-- Navigate to edit page
      handleClose();
  ...
  const handleDelete = async () => {
    await dispatch(deleteCoupons(couponId))
      .then(async (response) => {
      ...
      >
        {renderOptions
          ...
          : options.map((option) => (
              <MenuItem
                key={option}
                onClick={() => handleOtpions(option, couponId)}
              >
                {option}
              </MenuItem>
            ))}
