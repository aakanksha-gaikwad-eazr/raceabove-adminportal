...
export const getOrganizers = createAsyncThunk('appOrganizers/getOrganizers', async () => {
  try {
...
export const getSingleOrganizers = createAsyncThunk('appOrganizers/getSingleOrganizers', async (id) => {
  try {
    const adminData = JSON.parse(localStorage.getItem('raceabove'))
    const accessToken = adminData.accessToken
...
//add organizer
export const createOrganizer = createAsyncThunk('appOrganizers/createOrganizer', async (formData) => {
  try {
    const adminData = JSON.parse(localStorage.getItem('raceabove'))
    const accessToken = adminData.accessToken
...
//delete organizer
export const deleteOrganizer = createAsyncThunk('appOrganizers/deleteOrganizer', async id => {
  try {
    const adminData = JSON.parse(localStorage.getItem('raceabove'))
    const accessToken = adminData.accessToken
...
export const updateOrganizer = createAsyncThunk('appOrganizers/updateOrganizer', async (req) => {
  try {
    const adminData = JSON.parse(localStorage.getItem('raceabove'))
    const accessToken = adminData.accessToken
    ...
    console.error("❌ organizer update Failed:", error.response?.data || error.message);
