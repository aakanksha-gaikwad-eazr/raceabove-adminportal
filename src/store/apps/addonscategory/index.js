...
export const getAddOnsCategory = createAsyncThunk(
  "appAddOns/getAddOnsCategory",
  async () => {
    try {
      ...
      let url = `${ip}/v2/product-categories`;
...
export const reviewAddOnsCategory = createAsyncThunk(
  "appAddOns/reviewAddOnsCategory",
  async (req) => {
    try {
      const adminData = JSON.parse(localStorage.getItem("raceabove"));
      const accessToken = adminData.accessToken;
      ...
      let url = `${ip}/v2/product-categories/${req.id}/review`;
...
export const getSingleAddOnCategory = createAsyncThunk(
  "appAddOns/getSingleAddOnCategory",
  async (id) => {
    try {
      const adminData = JSON.parse(localStorage.getItem("raceabove"));
      const accessToken = adminData.accessToken;
      ...
      let url = `${ip}/v2/product-categories/${id}`;
